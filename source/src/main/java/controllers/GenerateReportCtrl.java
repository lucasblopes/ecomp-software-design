...
public class GenerateReportCtrl {
    ...
    public void generateReport() {
        ...
        if (reportType.equals("1")) {
            ...
            String reportContent = getFinancialReportContent();
            if (reportDestination.equals("1")) {
                view.displayReport(reportContent);
            } else if (reportDestination.equals("2")) {
                String filePath = "relatorio_financeiro.pdf";
                boolean success = saveReportToFile(reportContent, filePath);
                view.displayFileSaveResult(success, filePath);
            }
        }
    ...
    private String getFinancialReportContent() {
        StringBuilder report = new StringBuilder();
        report.append("===== RELATÓRIO FINANCEIRO =====\n\n");
            ...
            for (Activity activity : activityRepo.getActivities()) {
                if (activity.getInvoices() != null) {
                    for (Invoice invoice : activity.getInvoices()) {
                        report.append("   Nota Fiscal - Data: ").append(invoice.getDate())
                                .append(" | Valor: R$ ").append(invoice.getValue())
                                .append(" | Tipo: ").append(invoice.getType()).append("\n");
                        totalValue += invoice.getValue();
                    }
                }
            }
        ...
        report.append("TOTAL FINANCEIRO: R$ ").append(totalValue).append("\n");
        return report.toString();
    }
